...
public class Accion {
    ...
    public Accion(int id, String args, String cond, Mapa map) {
        this.setId(id);
        this.setArgs(args);
        this.setCond(cond);
        this.setMap(map);
    ...
    public static java.util.Map<Integer, Doble<Integer, Integer>> getDopeul() {
        java.util.Map<Integer, Doble<Integer, Integer>> changeDopeul = new HashMap<>();
    ...
    private static Doble<Integer, Integer> Couple(int i, int j) {
        return new Doble<>(i, j);
    }
    ...
    public boolean apply(final Jugador player, Jugador target, int itemID,
                         int cellid) {

        if (player == null)
            return true;
        if (player.getPelea() != null) {
            GestorSalida.GAME_SEND_MESSAGE(player, "<b>Action impossible,</b> vous êtes en combat !", "000000");
            return true;
        }
        if (!cond.equalsIgnoreCase("") && !cond.equalsIgnoreCase("-1") && !Mundo.mundo.getConditionManager().validConditions(player, cond)) {
            GestorSalida.GAME_SEND_Im_PACKET(player, "119");
            return true;
        }

        JuegoCliente client = player.getGameClient();
        switch (id) {
            case -22: //Remettre prisonnier
                if (player.getObjetByPos(Constantes.ITEM_POS_PNJ_SUIVEUR) != null) {
                    int skinFollower = player.getObjetByPos(Constantes.ITEM_POS_PNJ_SUIVEUR).getModelo().getId();
                    int questId = Constantes.getQuestByMobSkin(skinFollower);
                    if (questId != -1) {
                        //perso.upgradeQuest(questId);
                        player.setMascotte(1);
                        int itemFollow = Constantes.getItemByMobSkin(skinFollower);
                        player.removeByTemplateID(itemFollow, 1);
                    }
                }
                break;
            case -11: //T�l�portation map d�part � la cr�ation d'un personnage (?)
                player.teleport(Constantes.getStartMap(player.getClasse()), Constantes.getStartCell(player.getClasse()));
                GestorSalida.GAME_SEND_WELCOME(player);
                break;
            case -10: //Alignement ange si plus de demon et vice-versa sinon random
                ...
                int demon = 0;
                int total = 0;
                for (Jugador i : Mundo.mundo.getJugador()) {
                    if (i == null)
                        continue;
                    if (i.get_align() == 1)
                        ange++;
                ...
                else player.modifAlignement(Formulas.getRandomValue(1, 2));
                break;
            ...
            case -6://Dopeuls
                Mapa mapActuel = player.getCurMap();
                java.util.Map<Integer, Doble<Integer, Integer>> dopeuls = Accion.getDopeul();
                Integer IDmob = null;
                if (dopeuls.containsKey((int) mapActuel.getId())) {
                    IDmob = dopeuls.get((int) mapActuel.getId()).getPrimero();
                } else {
                    GestorSalida.GAME_SEND_MESSAGE(player, "Erreur de dopeul, veuillez nous avertir sur le forum.");
                    return true;
                }

                int LVLmob = Formulas.getLvlDopeuls(player.getLevel());
                if (player.getLevel() < 11) {
                    GestorSalida.GAME_SEND_MESSAGE(player, "Il faut être niveau 11 minimum pour combattre les dopeuls des temples.");
                    return true;
                }
                int certificat = Constantes.getCertificatByDopeuls(IDmob);
                if (certificat == -1)
                    return true;
                if (player.hasItemTemplate(certificat, 1)) {
                    ...
                    if (Instant.now().toEpochMilli() - timeStamp <= 86400000) {
                        GestorSalida.GAME_SEND_MESSAGE(player, "Il faut que tu attende 24 heures avant de pouvoir combattre ce dopeul.");
                        return true;
                    } else
                ...
                if (player.getQuestPerso() != null
                    ...
                    for (Entry<Integer, MisionJugador> entry : new HashMap<>(player.getQuestPerso()).entrySet()) {
                        MisionJugador qa = entry.getValue();
                        if (qa.getQuest().getId() == dopeuls.get((int) mapActuel.getId()).getSegundo()) {
                            b = false;
                            if (qa.isFinish()) {
                                player.delQuestPerso(entry.getKey());
                                if (qa.removeQuestPlayer()) {
                                    Mision q = Mision.getQuestById(dopeuls.get((int) mapActuel.getId()).getSegundo());
                                    q.applyQuest(player);
                                }
                            }
                        }
                ...
                Monstruos.MobGroup MG = new Monstruos.MobGroup(player.getCurMap().nextObjectId, player.getCurCell().getId(), grp);
                player.getCurMap().startFigthVersusDopeuls(player, MG);
                break;
            case -5://Apprendre un sort
                try {
                    int sID = Integer.parseInt(args);
                    if (Mundo.mundo.getSort(sID) == null)
                        return true;
...
//Payer
                    case 1 -> player.leaveEnnemyFactionAndPay(player);
//Attendre 10minutes
                    case 2 -> player.leaveEnnemyFaction();
                }
                break;
            case -3://Mascotte
            ...
            case -2://cr�er guilde
                ...
                if (player.hasItemTemplate(1575, 1)) {
                    ...
                    GestorSalida.GAME_SEND_MESSAGE(player, "Pour pouvoir créer une guilde, il faut posséder une Guildalogemme.");
                }
                break;
            ...
            case 1://Discours NPC
                ...
                if (args.equalsIgnoreCase("DV")) {
                    GestorSalida.GAME_SEND_END_DIALOG_PACKET(client);
                    ...
                    NpcPregunta quest = Mundo.mundo.getNPCQuestion(qID);
                    if (quest == null) {
                        GestorSalida.GAME_SEND_END_DIALOG_PACKET(client);
                        player.setExchangeAction(null);
                        return true;
                    ...
                    try {
                        GestorSalida.GAME_SEND_QUESTION_PACKET(client, quest.parse(player));
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
            ...
            case 4://Kamas
                try {
                    ...
                    if (newKamas < 0) {
                        GestorSalida.GAME_SEND_Im_PACKET(player, "084;1");
                        return true;
                    } else {
                        player.setKamas(newKamas);
                        GestorSalida.GAME_SEND_Im_PACKET(player, "046;" + count);
                        if (player.isOnline())
            ...
            case 5://objet
                try {
                    ...
                    if (count > 0) {
                        ...
                        if (T == null)
                            return true;
                        ObjetoJuego O = T.createNewItem(count, false);
                    ...
                    if (player.isOnline())//on envoie le packet qui indique l'ajout//retrait d'un item
                    {
                        ...
                        if (send) {
                            if (count >= 0) {
                                GestorSalida.GAME_SEND_Im_PACKET(player, "021;"
                                        + count + "~" + tID);
                            } else {
            ...
            case 6://Apprendre un m�tier
                try {
                    ...
                    if (Mundo.mundo.getMetier(mID) == null)
                        return true;
                    ...
                    if (mID == 2 || mID == 11 || mID == 13 || mID == 14
                        ...
                        if (player.getMetierByID(2) != null
                            ...
                            if (sucess == -1 || fail == -1) {
                                ...
                                GestorSalida.send(client, "DQ" + fail + "|4840");
                            return true;
                        }
                        ...
                        //Si c'est < ou = � 2 on apprend
                        {
                            if (mID == 27) {
                                ...
                                GestorSalida.GAME_SEND_Im_PACKET(player, "022;"
                                        + 966 + "~" + 1);
                                player.learnJob(Mundo.mundo.getMetier(mID));
                            } else {
                                ...
                                if (sucess == -1 || fail == -1) {
                                    ...
                                    GestorSalida.send(client, "DQ" + sucess
                                            + "|4840");
                            }
                        }
                    }
            ...
            case 8://Ajouter une Stat
                try {
                    ...
                    player.getCaracteristicas().addOneStat(statID, number);
                    GestorSalida.GAME_SEND_STATS_PACKET(player);
                    int messID = switch (statID) {
                        case Constantes.STATS_ADD_INTE -> 14;
                        default -> 0;
                    };
                    if (messID > 0)
                        GestorSalida.GAME_SEND_Im_PACKET(player, "0" + messID
                                + ";" + number);
                } catch (Exception e) {
                    e.printStackTrace();
                    return true;
            ...
            case 11://Definir l'alignement
                try {
                    ...
                    if (player.get_align() != Constantes.ALINEAMIENTO_NEUTRAL && !replace)
                        return true;
            ...
            case 12://Spawn d'un groupe de monstre
                try {
                    ...
                    PiedraAlma pierrePleine = (PiedraAlma) Mundo.getGameObject(itemID);
            ...
            case 13: //Reset de caracteristicas en Incarnam (Hada Risitas)
                if (player.getLevel() <= Configuracion.INSTANCE.getResetincarnam()) {
                    try {
                    ...
                    GestorSalida.GAME_SEND_Im_PACKET(player, "1267;" + Configuracion.INSTANCE.getResetincarnam());
                }
                break;

            case 14://Reset de hechizos Incarnam (Hada Risitas)
                if (player.getLevel() <= Configuracion.INSTANCE.getResetincarnam()) {
                    ...
                    if (ObjetNeed == 0) {
                        ...
                        if (MapNeed == 0) {
                            ...
                            if (player.hasItemTemplate(ObjetNeed, 1)
                                ...
                                GestorSalida.GAME_SEND_MESSAGE(player, "Vous n'êtes pas sur la bonne map du donjon pour être téléporter.", "009900");
                            } else {
                                //Si la persona no posee la llave necesaria
                                GestorSalida.GAME_SEND_MESSAGE(player, "Vous ne possédez pas la clef nécessaire.", "009900");
                            }
                        }
                    }
                } catch (Exception e) {
            ...
            case 18://T�l�portation chez soi
                if (Mundo.mundo.getHouseManager().alreadyHaveHouse(player))//Si il a une maison
                {
                    ObjetoJuego obj2 = Mundo.getGameObject(itemID);
                    if (player.hasItemTemplate(obj2.getModelo().getId(), 1)) {
                        player.removeByTemplateID(obj2.getModelo().getId(), 1);
                        Casas h = Mundo.mundo.getHouseManager().getHouseByPerso(player);
                        if (h == null)
                            return true;
            ...
            case 19://T�l�portation maison de guilde (ouverture du panneau de guilde)
                GestorSalida.GAME_SEND_GUILDHOUSE_PACKET(player);
                break;
            ...
            case 23://UnlearnJob
                ...
                if (Job < 1)
                    return true;
                OficioCaracteristicas m2 = player.getMetierByID(Job);
                if (m2 == null)
                    return true;
            ...
            case 24://Morph
                try {
                    ...
                    GestorSalida.GAME_SEND_ADD_PLAYER_TO_MAP(player.getCurMap(), player);
                } catch (Exception e) {
                    e.printStackTrace();
                    JuegoServidor.a();
                }
            ...
            case 26://T�l�portation enclos de guilde (ouverture du panneau de guilde)
                GestorSalida.GAME_SEND_GUILDENCLO_PACKET(player);
                break;
            ...
            case 27://Lancement de combat : startFigthVersusMonstres args : monsterID,monsterLevel| ...
                ...
                try {
                    ...
                    for (String MobAndLevel : args.split(":", 2)[0].split("\\|")) {
                        ...
                        if (Mundo.mundo.getMonstre(monsterID) == null
                                || Mundo.mundo.getMonstre(monsterID).getGradeByLevel(monsterLevel) == null) {
                            continue;
                        }
                        ValidMobGroup.append(monsterID).append(",").append(monsterLevel).append(",").append(monsterLevel).append(";");
                    }
                    if (ValidMobGroup.length() == 0)
                        return true;
                    Monstruos.MobGroup group = new Monstruos.MobGroup(player.getCurMap().nextObjectId, player.getCurCell().getId(), ValidMobGroup.toString());
            ...
            case 29://Desapprendre un sort avec kamas
                ...
                if (pKamas3 >= payKamas) {
                    ...
                    if (AncLevel <= 1)
                        return true;
                    player.unlearnSpell(player, sID, 1, AncLevel, true, true);
                    GestorSalida.GAME_SEND_MESSAGE(player, "Tu as perdu "
                            + payKamas + "Kamas.");
                } else {
                    GestorSalida.GAME_SEND_MESSAGE(player, "Tu n'as pas assez de kamas pour effectué cette action.");
                    return true;
                }
                break;
            ...
            case 31:// change classe(zobal)
                GestorSalida.GAME_SEND_MESSAGE(player, "Vous vous êtes fait enculer. Cordialement, le staff.");
				/*
            ...
            case 33:// Stat max a un obj pos
                ...
                ObjetoJuego itemPos = player.getObjetByPos(posItem);
                if (itemPos != null) {
                    itemPos.clearStats();
                    Caracteristicas maxStats = itemPos.generateNewStatsFromTemplate(itemPos.getModelo().getStrTemplate(), true);
                    itemPos.setCaracteristicas(maxStats);
                    int idObjPos = itemPos.getId();
                    player.removeItem(itemID, 1, true, true);
                    GestorSalida.GAME_SEND_REMOVE_ITEM_PACKET(player, idObjPos);
                    GestorSalida.GAME_SEND_OAKO_PACKET(player, itemPos);
                    GestorSalida.GAME_SEND_STATS_PACKET(player);
                } else {
                    GestorSalida.GAME_SEND_Im_PACKET(player, "Il n'y a aucune item dans la pos choisi.");
                }
                break;
            ...
            case 34://Loterie
                ...
                //int mapId1 = Integer.parseInt(args.split(",", 2)[1]);
                Loteria.startLoterie(player, idLot);
                break;
            ...
            case 35: //Reset Carac condition : Map x�lor 741 et l'obre de recons 10563
                try {
                    ...
                    GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 10563
                            + "~" + 1);
                    player.getCaracteristicas().addOneStat(125, -player.getCaracteristicas().getEffect(125));
                    player.getCaracteristicas().addOneStat(124, -player.getCaracteristicas().getEffect(124));
                    player.getCaracteristicas().addOneStat(118, -player.getCaracteristicas().getEffect(118));
                ...
                try {
                    ...
                    if (tutorial == 30) {
                        ...
                        if (random == 100)
                            ...
                            Database.estaticos.getNpcQuestionData().updateLot();
                    }
                    final Tutoriales tuto = Mundo.mundo.getTutorial(tutorial);
                    if (tuto == null)
                        return true;
                    if (player.getKamas() >= price) {
                        if (price != 0L) {
                            ...
                            if (player.isOnline())
                                GestorSalida.GAME_SEND_STATS_PACKET(player);
                            GestorSalida.GAME_SEND_Im_PACKET(player, "046;"
                                    + price);
                        }
                        try {
                            tuto.start.apply(player, null, -1, (short) -1);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }

                        Temporizador.addSiguiente(() -> {
                            GestorSalida.send(player, "TC" + tuto.id + "|7001010000");
                            player.setExchangeAction(new AccionIntercambiar<Object>(AccionIntercambiar.IN_TUTORIAL, tuto));
                            player.setAway(true);
                        }, 1500, Temporizador.DataType.CLIENTE);
                        return true;
                    }
                    GestorSalida.GAME_SEND_Im_PACKET(player, "182");
                } catch (Exception e23) {
                    e23.printStackTrace();
                }
                break;
            ...
            case 37://Loterie pioute
                Loteria.startLoteriePioute(player);
                break;
            ...
            case 40: //Donner une qu�te
                ...
                Mision quest0 = Mision.getQuestById(QuestID);
                if (quest0 == null) {
                    GestorSalida.GAME_SEND_MESSAGE(player, "La quête est introuvable.");
                    problem = true;
                    break;
                }
                for (MisionJugador qPerso : player.getQuestPerso().values()) {
                    if (qPerso.getQuest().getId() == QuestID) {
                        GestorSalida.GAME_SEND_MESSAGE(player, "Vous connaissez déjà cette quête.");
                        problem = true;
                        break;
                    }
                }
            ...
            case 43: //T�l�portation de qu�te
                ...
                if (player.getCurMap().getId() != mapsecu)
                    return true;
                MisionJugador questt = player.getQuestPersoByQuestId(questId);
            ...
            case 50: //Traque
                ...
                if (player.get_traque() != null && player.get_traque().getTime() == -2) {
                    ...
                    GestorSalida.GAME_SEND_STATS_PACKET(player);
                    GestorSalida.GAME_SEND_MESSAGE(player, "Vous venez de recevoir " + xp + " points d'experiences.", "000000");
                    return true;
                } else if(player.get_traque() != null) {
                    GestorSalida.GAME_SEND_MESSAGE(player, "Thomas Sacre : Reviens me voir quand tu aura abatu un ennemi.", "000000");
                    return true;
                }
                ...
                if (player.get_traque().getTime() < Instant.now().toEpochMilli() - 600000 || player.get_traque().getTime() == 0) {
                    Jugador tempP = null;
                    ArrayList<Jugador> victimes = new ArrayList<>();
                    for (Jugador victime : Mundo.mundo.getOnlinePlayers()) {
                        if (victime == null || victime == player)
                            continue;
						if (victime.getAccount().getCurrentIp().compareTo(player.getAccount().getCurrentIp()) == 0)
                            continue;
                        if(player.restricciones.aggros.containsKey(victime.getAccount().getCurrentIp()))
                            continue;
                    ...
                    if (victimes.size() == 0) {
                        GestorSalida.GAME_SEND_MESSAGE(player, "Nous n'avons pas trouver de cible à ta hauteur, reviens plus tard.", "000000");
                    ...
                    GestorSalida.GAME_SEND_MESSAGE(player, "Vous êtes désormais en chasse de : " + tempP.getName(), "000000");
                    player.get_traque().setTraque(tempP);
                    player.get_traque().setTime(Instant.now().toEpochMilli());
                    ObjetoJuego object = player.getItemTemplate(10085);
                    if(object != null)
                        player.removeItem(object.getId(), player.getNbItemTemplate(10085), true, true);
                    ObjetoModelo T = Mundo.mundo.getObjetoModelo(10085);
                    ObjetoJuego newObj = T.createNewItem(20, false);
                    newObj.addTxtStat(Constantes.STATS_NAME_TRAQUE, tempP.getName());
                    newObj.addTxtStat(Constantes.STATS_ALIGNEMENT_TRAQUE, Integer.toHexString(tempP.get_align()) + "");
                    newObj.addTxtStat(Constantes.STATS_GRADE_TRAQUE, Integer.toHexString(tempP.getALvl()) + "");
                    newObj.addTxtStat(Constantes.STATS_NIVEAU_TRAQUE, Integer.toHexString(tempP.getLevel()) + "");

                    if (player.addObjet(newObj, true))
                        Mundo.addGameObject(newObj, true);
                } else {
                    GestorSalida.GAME_SEND_MESSAGE(player, "Vous venez juste de signer un contrat, vous devez vous reposez !", "000000");
                }
                break;
            ...
            case 53: //Suivre le d�placement pour une map pr�sice
                if (args == null)
                    break;
                if (Mundo.mundo.getMap(Short.parseShort(args)) == null)
                    break;
                Mapa CurMap = Mundo.mundo.getMap(Short.parseShort(args));
                if (player.getPelea() == null) {
                    GestorSalida.GAME_SEND_FLAG_PACKET(player, CurMap);
                }
                break;
            ...
            case 60: //Combattre un protecteur
                ...
                try {
                    for (String MobAndLevel : args.split("\\|")) {
                        ...
                        if (Mundo.mundo.getMonstre(monsterID) == null
                                || Mundo.mundo.getMonstre(monsterID).getGradeByLevel(lvlMin) == null
                                || Mundo.mundo.getMonstre(monsterID).getGradeByLevel(lvlMax) == null) {
                            continue;
                        }
                        ValidMobGroup1.append(monsterID).append(",").append(lvlMin).append(",").append(lvlMax).append(";");
                    }
                    if (ValidMobGroup1.length() == 0)
                        return true;
                    Monstruos.MobGroup group = new Monstruos.MobGroup(player.getCurMap().nextObjectId, player.getCurCell().getId(), ValidMobGroup1.toString());
            ...
            case 100: //Donner l'abilit� 'args' � une dragodinde
                if (player.hasItemTemplate(361, 100)) {
                    ...
                    ObjetoJuego newObjAdded = Mundo.mundo.getObjetoModelo(9201).createNewItem(1, false);
            ...
            case 102: //Marier des personnages
                ...
                if(map0.getCase(297).getPlayers() != null && map0.getCase(282).getPlayers() != null) {
                    if (map0.getCase(297).getPlayers().size() == 1 && map0.getCase(282).getPlayers().size() == 1) {
                        Jugador boy = (Jugador) map0.getCase(282).getPlayers().toArray()[0], girl = (Jugador) map0.getCase(297).getPlayers().toArray()[0];
                        boy.setBlockMovement(true);
                        girl.setBlockMovement(true);
                        Mundo.mundo.priestRequest(boy, girl, player);
                    }
                }
                break;
            ...
            case 103: //Divorce
                if (player.getKamas() < 50000) {
                    ...
                    if (player.isOnline())
                        GestorSalida.GAME_SEND_STATS_PACKET(player);
                    Jugador wife = Mundo.mundo.getPlayer(player.getWife());
                    wife.Divorce();
                    player.Divorce();
                }
                break;
            ...
            case 104: //T�l�portation mine + Animation
                ...
                ArrayList<Doble<Short, Integer>> arrays = new ArrayList<>();
                for (String i : args.split(";"))
                    arrays.add(new Doble<>(Short.parseShort(i.split(",")[0]), Integer.parseInt(i.split(",")[1])));

                Doble<Short, Integer> couple = arrays.get(Formulas.random.nextInt(arrays.size()));
                GestorSalida.GAME_SEND_GA_PACKET(player.getGameClient(), "", "2", player.getId()
                        + "", "6");
                player.teleport(couple.getPrimero(), couple.getSegundo());
                break;
            ...
            case 105: //Restat carac second pouvoir
                if (!player.hasItemTemplate(10563, 1)) {
                    ...
                    player.removeByTemplateID(10563, 1);
                    GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 10563 + "~" + 1);
                }
                ...
                if (player.getStatsParcho().getEffect(125) != 0
                        ...
                        || player.getCaracteristicas().getEffect(123) != 0) {
                    player.getCaracteristicas().addOneStat(125, -player.getCaracteristicas().getEffect(125)
                    ...
                    player.addCapital((player.getLevel() - 1) * 5
                            - player.get_capital());
                } else if (player.getCaracteristicas().getEffect(125) == 101
                        && player.getCaracteristicas().getEffect(124) == 101
                        && player.getCaracteristicas().getEffect(118) == 101
                        && player.getCaracteristicas().getEffect(123) == 101
                        && player.getCaracteristicas().getEffect(119) == 101
                        && player.getCaracteristicas().getEffect(126) == 101) {
                    player.getCaracteristicas().addOneStat(125, -player.getCaracteristicas().getEffect(125) + 101);
                    player.getCaracteristicas().addOneStat(124, -player.getCaracteristicas().getEffect(124) + 101);
                    player.getCaracteristicas().addOneStat(118, -player.getCaracteristicas().getEffect(118) + 101);
                    player.getCaracteristicas().addOneStat(123, -player.getCaracteristicas().getEffect(123) + 101);
                    player.getCaracteristicas().addOneStat(119, -player.getCaracteristicas().getEffect(119) + 101);
                    player.getCaracteristicas().addOneStat(126, -player.getCaracteristicas().getEffect(126) + 101);
                    ...
                    case "3"://Restat with
                        if(player.hasItemTemplate(15005, 1)) {
                            ...
                            if (player.getStatsParcho().getEffect(125) != 0 || player.getStatsParcho().getEffect(124) != 0 || player.getStatsParcho().getEffect(118) != 0
                                ...
                                player.getCaracteristicas().addOneStat(125, -player.getCaracteristicas().getEffect(125) + player.getStatsParcho().getEffect(125));
                                player.getCaracteristicas().addOneStat(124, -player.getCaracteristicas().getEffect(124) + player.getStatsParcho().getEffect(124));
                                player.getCaracteristicas().addOneStat(118, -player.getCaracteristicas().getEffect(118) + player.getStatsParcho().getEffect(118));
                                player.getCaracteristicas().addOneStat(123, -player.getCaracteristicas().getEffect(123) + player.getStatsParcho().getEffect(123));
                                player.getCaracteristicas().addOneStat(119, -player.getCaracteristicas().getEffect(119) + player.getStatsParcho().getEffect(119));
                                player.getCaracteristicas().addOneStat(126, -player.getCaracteristicas().getEffect(126) + player.getStatsParcho().getEffect(126));
                                player.addCapital((player.getLevel() - 1) * 5 - player.get_capital());
                                GestorSalida.GAME_SEND_STATS_PACKET(player);
                            } else if (player.getCaracteristicas().getEffect(125) == 101 && player.getCaracteristicas().getEffect(124) == 101 && player.getCaracteristicas().getEffect(118) == 101
                                    && player.getCaracteristicas().getEffect(123) == 101 && player.getCaracteristicas().getEffect(119) == 101 && player.getCaracteristicas().getEffect(126) == 101) {
                                player.getCaracteristicas().addOneStat(125, -player.getCaracteristicas().getEffect(125) + 101);
                                player.getCaracteristicas().addOneStat(124, -player.getCaracteristicas().getEffect(124) + 101);
                                player.getCaracteristicas().addOneStat(118, -player.getCaracteristicas().getEffect(118) + 101);
                                player.getCaracteristicas().addOneStat(123, -player.getCaracteristicas().getEffect(123) + 101);
            ...
            case 116://EPO donner de la nourriture � son familier
                ObjetoJuego EPO = Mundo.getGameObject(itemID);
                if (EPO == null)
                    return true;
                ...
                if (MyPets == null)
                    return true;
                if (EPO.getModelo().getConditions().contains(pets.getModelo().getId()
                        + ""))
            ...
            case 170:// Donner titre
                try {
                    ...
                    target = Mundo.mundo.getPlayerByName(player.getName());
                    target.set_title(title1);
                    GestorSalida.GAME_SEND_MESSAGE(player, "Vous avez désormais un nouveau titre.");
                    GestorSalida.GAME_SEND_STATS_PACKET(player);
                    Database.dinamicos.getPlayerData().update(player);
            ...
            case 171: // Alignement avec condition
                ...
                if (type2 == 1 && player.getCurMap().getId() == mapId2) {
                    if (player.hasItemTemplate(42, 10)) {
                        ...
                        GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 42 + "~" + 10);
                ...
                if (type2 == 2 && player.getCurMap().getId() == mapId2) {
                    if (player.hasItemTemplate(95, 10)) {
                        ...
                        GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 95 + "~" + 10);
            ...
            case 172: //Bricoleur avec condition
                ...
                if (player.hasItemTemplate(459, 20)
                    ...
                    GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 459 + "~"
                            + 20);
                    player.removeByTemplateID(7657, 15);
                    GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 7657
                            + "~" + 15);
                    player.learnJob(Mundo.mundo.getMetier(65));
                } else {
                    GestorSalida.GAME_SEND_Im_PACKET(player, "14|43");
                    return true;
                }
                break;
            ...
            case 200: //Animation + Condition voyage Ile Minotor
                ...
                if (pKamas2 >= 100 && player.getCurMap().getId() == 9520) {
                    ...
                    if (player.isOnline())
                        GestorSalida.GAME_SEND_STATS_PACKET(player);
                    GestorSalida.GAME_SEND_Im_PACKET(player, "046;" + 100);
                    GestorSalida.GAME_SEND_GA_PACKET(player.getGameClient(), "", "2", player.getId()
                            + "", "2");
            ...
            case 219://Sortie donjon wabbit
                ...
                if (type11 == 1) {
                    ObjetoJuego newObjAdded = Mundo.mundo.getObjetoModelo(970).createNewItem(1, false);
                    if (!player.addObjetSimiler(newObjAdded, true, -1)) {
                        Mundo.addGameObject(newObjAdded, true);
                        player.addObjet(newObjAdded);
                    }
                    GestorSalida.GAME_SEND_Im_PACKET(player, "021;" + 1 + "~"
                            + 970);
                    player.teleport((short) 844, 212);
                } else if (type11 == 2) {
                    ObjetoJuego newObjAdded = Mundo.mundo.getObjetoModelo(969).createNewItem(1, false);
                    if (!player.addObjetSimiler(newObjAdded, true, -1)) {
                        Mundo.addGameObject(newObjAdded, true);
                        player.addObjet(newObjAdded);
                    }
                    ...
                    ObjetoJuego newObjAdded = Mundo.mundo.getObjetoModelo(971).createNewItem(1, false);
                    if (!player.addObjetSimiler(newObjAdded, true, -1)) {
                        Mundo.addGameObject(newObjAdded, true);
                        player.addObjet(newObjAdded);
                    }
            ...
            case 220:// IDitem,quantit� pour IDitem's,quantit� pour teleport
                try {
                    ...
                    if (player.hasItemTemplate(obj0, qua0)) {
                        ...
                        GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + qua0
                                + "~" + obj0);
                        GestorSalida.GAME_SEND_Im_PACKET(player, "021;"
                                + newQua1 + "~" + newObj1);
                        GestorSalida.GAME_SEND_Im_PACKET(player, "021;"
                                + newQua2 + "~" + newObj2);
                        ObjetoJuego newObjAdded = Mundo.mundo.getObjetoModelo(newObj1).createNewItem(newQua1, false);
                        if (!player.addObjetSimiler(newObjAdded, true, -1)) {
                            Mundo.addGameObject(newObjAdded, true);
                            player.addObjet(newObjAdded);
                        }
                        ObjetoJuego newObjAdded1 = Mundo.mundo.getObjetoModelo(newObj2).createNewItem(newQua2, false);
                        if (!player.addObjetSimiler(newObjAdded1, true, -1)) {
                            Mundo.addGameObject(newObjAdded1, true);
            ...
            case 221:// IDitem,quantit� pour IDitem's,quantit� pour teleport
                try {
                    ...
                    if (player.hasItemTemplate(obj0, qua0)
                        ...
                        GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + qua0
                                + "~" + obj0);
                        GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + qua1
                                + "~" + obj1);
                        GestorSalida.GAME_SEND_Im_PACKET(player, "021;"
                                + newQua1 + "~" + newObj1);
                        ObjetoJuego newObjAdded = Mundo.mundo.getObjetoModelo(newObj1).createNewItem(newQua1, false);
            ...
            case 222:// IDitem,quantit� pour IDitem's,quantit� pour teleport
                try {
                    ...
                    if (player.hasItemTemplate(obj0, qua0)
                        ...
                        GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + qua1
                                + "~" + obj1);
                        GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + qua2
                                + "~" + obj2);
                        GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + qua3
                                + "~" + obj3);
                        if (player.getPelea() != null
            ...
            case 223:// IDitem,quantit� pour IDitem's,quantit�
                try {
                    ...
                    if (player.hasItemTemplate(obj0, qua0)
                        ...
                        ObjetoJuego newObjAdded = Mundo.mundo.getObjetoModelo(newItem).createNewItem(quaNewItem, false);
                        if (!player.addObjetSimiler(newObjAdded, true, -1)) {
                            Mundo.addGameObject(newObjAdded, true);
                            player.addObjet(newObjAdded);
                        }
                        ...
                        GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + qua3
                                + "~" + obj3);
                        GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + qua4
                                + "~" + obj4);
                        GestorSalida.GAME_SEND_Im_PACKET(player, "021;"
                                + quaNewItem + "~" + newItem);
                    } else {
            ...
            case 226://Animation + Condition voyage Ile Moon
                if (player.hasItemTemplate(1089, 1) && player.hasEquiped(1021)
                    ...
                    ObjetoJuego newObj1 = Mundo.mundo.getObjetoModelo(1020).createNewItem(1, false);
                    if (!player.addObjetSimiler(newObj1, true, -1)) {
                        Mundo.addGameObject(newObj1, true);
                        player.addObjet(newObj1);
                    }
                    ObjetoJuego newObj2 = Mundo.mundo.getObjetoModelo(1022).createNewItem(1, false);
                    if (!player.addObjetSimiler(newObj2, true, -1)) {
                        Mundo.addGameObject(newObj2, true);
            ...
            case 227://Animation + Condition voyage Ile Wabbit
                ...
                if (pKamas >= 500 && player.getCurMap().getId() == 167) {
                    ...
                    if (player.isOnline())
                        GestorSalida.GAME_SEND_STATS_PACKET(player);
                    GestorSalida.GAME_SEND_Im_PACKET(player, "046;" + 500);
                    GestorSalida.GAME_SEND_GA_PACKET(player.getGameClient(), "", "2", player.getId()
                            + "", "2");
            ...
            case 228://Faire animation Hors Combat
                try {
                    ...
                    Animaciones animation = Mundo.mundo.getAnimation(AnimationId);
                    if (player.getPelea() != null)
                        return true;
                    player.changeOrientation(1);
                    GestorSalida.GAME_SEND_GA_PACKET_TO_MAP(player.getCurMap(), "0", 228, player.getId()
            ...
            case 229://Animation d'incarnam � astrub
                short map = Constantes.getClassStatueMap(player.getClasse());
                int cell = Constantes.getClassStatueCell(player.getClasse());
                GestorSalida.GAME_SEND_GA_PACKET(player.getGameClient(), "", "2", player.getId()
                        + "", "7");
            ...
            case 230://Point Boutique  
                try {
                    ...
                    if (player.isOnline())
                        GestorSalida.GAME_SEND_STATS_PACKET(player);
                    GestorSalida.GAME_SEND_MESSAGE(player, "Tu viens d'acquérir "
                            + pts
            ...
            case 231:// IDitem,quantit� pour IDitem's,quantit�
                try {
                    ...
                    if (player.hasItemTemplate(obj, qua)) {
                        ...
                        GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + qua
                                + "~" + obj);
                        GestorSalida.GAME_SEND_Im_PACKET(player, "021;"
                                + quaNewItem + "~" + newItem);
                    } else {
                ...
                if (pMap == 10131 || pMap == 10132 || pMap == 10133
                    ...
                    try {
                        for (String MobAndLevel : args.split("\\|")) {
                            ...
                            ValidMobGroup2.append(monsterID).append(",").append(monsterLevel).append(",").append(monsterLevel).append(";");
                        }
                        if (ValidMobGroup2.length() == 0)
                            return true;
                        Monstruos.MobGroup group = new Monstruos.MobGroup(player.getCurMap().nextObjectId, player.getCurCell().getId(), ValidMobGroup2.toString());
                        player.getCurMap().startFightVersusMonstres(player, group);// Si bug startfight, voir "//Respawn d'un groupe fix" dans fight.java
                    } catch (Exception e) {
                        e.printStackTrace();
                        JuegoServidor.a();
                    ...
                    GestorSalida.GAME_SEND_MESSAGE(player, "Vous ne pouvez pas vous battre ici. Allez en arène !");
                }
                break;
            ...
            case 234: //Ajout d'un objet en fonction de la map
                ...
                if (!player.hasItemTemplate(IdObj, 1)) {
                    ObjetoJuego newObjAdded = Mundo.mundo.getObjetoModelo(IdObj).createNewItem(1, false);
                    if (!player.addObjetSimiler(newObjAdded, true, -1)) {
                        Mundo.addGameObject(newObjAdded, true);
                        player.addObjet(newObjAdded);
                    }
                } else {
                    GestorSalida.GAME_SEND_MESSAGE(player, "Tu posséde déjà l'objet.");
                }
                break;
            ...
            case 235:// IDitem,quantit� pour IDitem's,quantit�
                if (player.getCurMap().getId() == 713) {
                    if (player.hasItemTemplate(757, 1)
                        ...
                        ObjetoJuego newObjAdded = Mundo.mundo.getObjetoModelo(960).createNewItem(1, false);
                        if (!player.addObjetSimiler(newObjAdded, true, -1)) {
                            Mundo.addGameObject(newObjAdded, true);
                            player.addObjet(newObjAdded);
                        }
            ...
            case 241:
                if (player.getKamas() >= 10
                    ...
                    if (player.hasItemTemplate(6653, 1)) {
                        String date = player.getItemTemplate(6653, 1).getTxtStat().get(Constantes.STATS_DATE);
                        long timeStamp = Long.parseLong(date.split("#")[3]);
                        if (Instant.now().toEpochMilli() - timeStamp <= 86400000) {
                            GestorSalida.GAME_SEND_MESSAGE(player, "Ton ticket est bon.");
                            return true;
                        } else {
                            GestorSalida.GAME_SEND_MESSAGE(player, "Ton ticket est dépassé, il faut que tu en rachéte un.");
                    ...
                    if (player.isOnline())
                        GestorSalida.GAME_SEND_STATS_PACKET(player);
                    ObjetoModelo OT = Mundo.mundo.getObjetoModelo(6653);
                    ObjetoJuego obj = OT.createNewItem(1, false);
            ...
            case 450: //Ortimus
                if (player.getCurMap().getId() == 1844
                    ...
                    if (player.isOnline())
                        GestorSalida.GAME_SEND_STATS_PACKET(player);
                    ObjetoJuego newObjAdded = Mundo.mundo.getObjetoModelo(998).createNewItem(1, false);
                    if (!player.addObjetSimiler(newObjAdded, true, -1)) {
                        Mundo.addGameObject(newObjAdded, true);
                        player.addObjet(newObjAdded);
                    }
                    GestorSalida.GAME_SEND_Im_PACKET(player, "021;" + 1 + "~"
                            + 998);
                    GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 5 + "~"
                            + 363);
                } else {
                    GestorSalida.GAME_SEND_Im_PACKET(player, "14|43");
                }
            ...
            case 451: //Employ� de l'agence Touriste
                if (player.getKamas() >= 200
                    ...
                    if (player.isOnline())
                        GestorSalida.GAME_SEND_STATS_PACKET(player);
                    ObjetoJuego newObjAdded = Mundo.mundo.getObjetoModelo(1004).createNewItem(1, false);
                    if (!player.addObjetSimiler(newObjAdded, true, -1)) {
                        Mundo.addGameObject(newObjAdded, true);
                        player.addObjet(newObjAdded);
                    }
            ...
            case 452: //Kib Roche
                if (player.hasItemTemplate(1000, 6)
                    ...
                    GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 6 + "~"
                            + 1000);
                    player.removeByTemplateID(1003, 1);
                    GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 1 + "~"
                            + 1003);
                    player.removeByTemplateID(1018, 10);
                    GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 10 + "~"
                            + 1018);
                    ...
                    GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 4 + "~"
                            + 1004);
                    player.removeByTemplateID(1001, 2);
                    GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 2 + "~"
                            + 1001);
                    ObjetoJuego newObjAdded = Mundo.mundo.getObjetoModelo(6716).createNewItem(1, false);
                    if (!player.addObjetSimiler(newObjAdded, true, -1)) {
                        Mundo.addGameObject(newObjAdded, true);
                        player.addObjet(newObjAdded);
                    }
            ...
            case 455://Iss� Heau : peinture noire : fonctionnel
                if (player.hasItemTemplate(1006, 1)
                    ...
                    GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 1 + "~"
                            + 1009);
                    ObjetoJuego newObjAdded = Mundo.mundo.getObjetoModelo(1086).createNewItem(1, false);
                    if (!player.addObjetSimiler(newObjAdded, true, -1)) {
                        Mundo.addGameObject(newObjAdded, true);
                        player.addObjet(newObjAdded);
                    }
            ...
            case 456: //Obtention du masque kanniboul : fonctionnel
                if (player.hasItemTemplate(1014, 1)
                    ...
                    GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 1 + "~"
                            + 1086);
                    ObjetoJuego newObjAdded = Mundo.mundo.getObjetoModelo(1088).createNewItem(1, false);
                    if (!player.addObjetSimiler(newObjAdded, true, -1)) {
                        Mundo.addGameObject(newObjAdded, true);
                        player.addObjet(newObjAdded);
                    }
                    ...
                    NpcPregunta quest = Mundo.mundo.getNPCQuestion(577);
                    if (quest == null) {
                        GestorSalida.GAME_SEND_END_DIALOG_PACKET(player.getGameClient());
                        player.setExchangeAction(null);
                        return true;
                    }
                    try {
                        GestorSalida.GAME_SEND_QUESTION_PACKET(player.getGameClient(), quest.parse(player));
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                } else {
            ...
            case 457://Vente ticket �le moon
                if (player.getKamas() >= 1000
                    ...
                    ObjetoJuego newObjAdded11 = Mundo.mundo.getObjetoModelo(1089).createNewItem(1, false);
                    if (!player.addObjetSimiler(newObjAdded11, true, -1)) {
                        Mundo.addGameObject(newObjAdded11, true);
            ...
            case 500:// Donjon Bouftou : R�compense.
                ...
                ObjetoJuego newObjAdded = Mundo.mundo.getObjetoModelo(1728).createNewItem(1, false);
                if (!player.addObjetSimiler(newObjAdded, true, -1)) {
                    Mundo.addGameObject(newObjAdded, true);
                    player.addObjet(newObjAdded);
                }
            ...
            case 501:// Donjon Bwork : R�compense.
                ...
                ObjetoJuego newObjAdded1 = Mundo.mundo.getObjetoModelo(8000).createNewItem(1, false);
                if (!player.addObjetSimiler(newObjAdded1, true, -1)) {
                    Mundo.addGameObject(newObjAdded1, true);
                    player.addObjet(newObjAdded1);
                }
            ...
            case 502://Entr� Donjon Cawotte
                if (player.hasEquiped(969) && player.hasEquiped(970)
                    ...
                    GestorSalida.GAME_SEND_Im_PACKET(player, "114;");
                }
                break;
            ...
            case 503://Sortie Donjon Cawotte
                ...
                if (player.hasItemTemplate(969, 1)
                    ...
                    ObjetoJuego newObjAdded11 = Mundo.mundo.getObjetoModelo(972).createNewItem(1, false);
                    if (!player.addObjetSimiler(newObjAdded11, true, -1)) {
                        Mundo.addGameObject(newObjAdded11, true);
                        player.addObjet(newObjAdded11);
                    }
            ...
            case 504://Sortie Donjon Koulosse
                if (player.getCurMap().getId() == 9717) {
                    ...
                    if (type111 == 1) {
                        ObjetoJuego newObjAdded11 = Mundo.mundo.getObjetoModelo(7890).createNewItem(1, false);
                        if (!player.addObjetSimiler(newObjAdded11, true, -1)) {
                            Mundo.addGameObject(newObjAdded11, true);
                            player.addObjet(newObjAdded11);
                        }
                    ...
                    if (type111 == 2) {
                        ObjetoJuego newObjAdded11 = Mundo.mundo.getObjetoModelo(7889).createNewItem(1, false);
                        if (!player.addObjetSimiler(newObjAdded11, true, -1)) {
                            Mundo.addGameObject(newObjAdded11, true);
                            player.addObjet(newObjAdded11);
                        }
                    ...
                    if (type111 == 3) {
                        ObjetoJuego newObjAdded11 = Mundo.mundo.getObjetoModelo(7888).createNewItem(1, false);
                        if (!player.addObjetSimiler(newObjAdded11, true, -1)) {
                            Mundo.addGameObject(newObjAdded11, true);
                            player.addObjet(newObjAdded11);
                        }
                    ...
                    if (type111 == 4) {
                        ObjetoJuego newObjAdded11 = Mundo.mundo.getObjetoModelo(7887).createNewItem(1, false);
                        if (!player.addObjetSimiler(newObjAdded11, true, -1)) {
                            Mundo.addGameObject(newObjAdded11, true);
                            player.addObjet(newObjAdded11);
                        }
            ...
            case 506://Entr� Donjon Koulosse
                if (player.getCurMap().getId() == 8905
                    ...
                    if (player.hasItemTemplate(7908, 1)) {
                        ...
                        GestorSalida.GAME_SEND_MESSAGE(player, "Vous ne possédez pas la clef neccéssaire.");
                    }
                } else {
                    GestorSalida.GAME_SEND_MESSAGE(player, "Vous n'êtes pas devant le PNJ.");
                }
                break;
                    ...
                    switch (type3) {
                        case 1://Menthe -> 2433
                            if (player.hasItemTemplate(2433, 10)) {
                                ...
                                GestorSalida.GAME_SEND_MESSAGE(player, "Vous n'avez pas le bon nombre de flaque de Menthe.");
                            }
                            break;
                        case 2://Fraise -> 2432
                            if (player.hasItemTemplate(2432, 10)) {
                                ...
                                GestorSalida.GAME_SEND_MESSAGE(player, "Vous n'avez pas le bon nombre de flaque de Fraise.");
                            }
                            break;
                        case 3://Citron -> 2431
                            if (player.hasItemTemplate(2431, 10)) {
                                ...
                                GestorSalida.GAME_SEND_MESSAGE(player, "Vous n'avez pas le bon nombre de flaque de Citron.");
                            }
                            break;
                        case 4://Bleue -> 2430
                            if (player.hasItemTemplate(2430, 10)) {
                                ...
                                GestorSalida.GAME_SEND_MESSAGE(player, "Vous n'avez pas le bon nombre de flaque de Bleue.");
                            }
                            break;
                        case 5://Gel�e x10 => 2433,2432,2431,2430
                            if (player.hasItemTemplate(2433, 10)
                                ...
                                GestorSalida.GAME_SEND_MESSAGE(player, "Vous n'avez pas le bon nombre de flaque pour combattre 2 gelées royales.");
                            }
                            break;
                        case 6://Menthe -> 2433
                            if (player.hasItemTemplate(2433, 15)
                                ...
                                GestorSalida.GAME_SEND_MESSAGE(player, "Vous n'avez pas le bon nombre de flaque pour combattre 4 gelées royales.");
                            }
                            break;
                    }
                }
            ...
            case 508://Donjon Kitsoune : R�compense
                if (player.getCurMap().getId() == 8317) {
                    ...
                    ObjetoJuego newObjAdded11 = Mundo.mundo.getObjetoModelo(7415).createNewItem(1, false);
                    if (!player.addObjetSimiler(newObjAdded11, true, -1)) {
                        Mundo.addGameObject(newObjAdded11, true);
                        player.addObjet(newObjAdded11);
                    }
            ...
            case 509://Donjon Bworker : R�compense
                ...
                ObjetoJuego newObjAdded11 = Mundo.mundo.getObjetoModelo(6885).createNewItem(1, false);
                if (!player.addObjetSimiler(newObjAdded11, true, -1)) {
                    Mundo.addGameObject(newObjAdded11, true);
                    player.addObjet(newObjAdded11);
                }
                GestorSalida.GAME_SEND_Im_PACKET(player, "021;" + 1 + "~"
                        + 6885);
                ObjetoJuego newObjAdded12 = Mundo.mundo.getObjetoModelo(8388).createNewItem(1, false);
                if (!player.addObjetSimiler(newObjAdded12, true, -1)) {
                    Mundo.addGameObject(newObjAdded12, true);
            ...
            case 510://Jeton Bwroker Vs Packet
                if (player.getCurMap().getId() == 3373
                    ...
                    ObjetoJuego newObjAdded121 = Mundo.mundo.getObjetoModelo(6887).createNewItem(1, false);
                    if (!player.addObjetSimiler(newObjAdded121, true, -1)) {
                        Mundo.addGameObject(newObjAdded121, true);
            ...
            case 511://Cadeau Bworker
                int cadeau = Loteria.getCadeauBworker();
                ObjetoJuego newObjAdded121 = Mundo.mundo.getObjetoModelo(cadeau).createNewItem(1, false);
                if (!player.addObjetSimiler(newObjAdded121, true, -1)) {
                    Mundo.addGameObject(newObjAdded121, true);
                    player.addObjet(newObjAdded121);
                }
            ...
            case 512://Rat Blanc : R�compense
                if (player.getCurMap().getId() == 10213) {
                    ...
                    ObjetoJuego newObjAdded111 = Mundo.mundo.getObjetoModelo(8476).createNewItem(1, false);
                    if (!player.addObjetSimiler(newObjAdded111, true, -1)) {
                        Mundo.addGameObject(newObjAdded111, true);
            ...
            case 513://Rat noir : R�compense
                if (player.getCurMap().getId() == 10199) {
                    ...
                    ObjetoJuego newObjAdded111 = Mundo.mundo.getObjetoModelo(8477).createNewItem(1, false);
                    if (!player.addObjetSimiler(newObjAdded111, true, -1)) {
                        Mundo.addGameObject(newObjAdded111, true);
                        player.addObjet(newObjAdded111);
                    }
            ...
            case 515://R�compense Pandikaze
                if (player.getCurMap().getId() == 8497) {
                    ...
                    ObjetoJuego newObjAdded111 = Mundo.mundo.getObjetoModelo(7414).createNewItem(1, false);
                    if (!player.addObjetSimiler(newObjAdded111, true, -1)) {
                        Mundo.addGameObject(newObjAdded111, true);
                        player.addObjet(newObjAdded111);
                    }
            ...
            case 516://Chenil achat poudre eniripsa
                if (player.getCurMap().getId() == 1140
                    ...
                    ObjetoJuego newObjAdded111 = Mundo.mundo.getObjetoModelo(2239).createNewItem(1, false);
                    if (!player.addObjetSimiler(newObjAdded111, true, -1)) {
                        Mundo.addGameObject(newObjAdded111, true);
                        player.addObjet(newObjAdded111);
                    }
            ...
            case 517://Entr�e donjon familier 9052,268 - O: 7 - Transformation donjon abra
                ...
                if (player.getCurMap().getId() == 9052) {
                    ...
                    if (!player.hasItemType(90)) {
                        GestorSalida.GAME_SEND_MESSAGE(player, "Vous n'avez pas de fantôme de familier.");
                        return true;
                    }
                }
                player.teleport((short) mapId, cellId);
            ...
            case 519://Donjon Grotte Hesque, Arche, Rasboul, Tynril
                ...
                if (player.getCurMap().getId() != mapSecu)
                    return true;
                ObjetoJuego obj1 = Mundo.mundo.getObjetoModelo(Integer.parseInt(args.split(";")[2])).createNewItem(1, false);
                if (obj1 != null)
                    if (player.addObjet(obj1, true))
                        Mundo.addGameObject(obj1, true);
                player.send("Im021;1~" + args.split(";")[2]);
                obj1 = Mundo.mundo.getObjetoModelo(Integer.parseInt(args.split(";")[3])).createNewItem(1, false);
                if (obj1 != null)
                    if (player.addObjet(obj1, true))
                        Mundo.addGameObject(obj1, true);
            ...
            case 520://Dj pandikaze
                ...
                obj1 = Mundo.mundo.getObjetoModelo(7414).createNewItem(1, false);
                if (player.addObjet(obj1, true))
                    Mundo.addGameObject(obj1, true);
                player.send("Im021;1~7414");
                if (!player.getEmotes().contains(15)) {
                    obj1 = Mundo.mundo.getObjetoModelo(7413).createNewItem(1, false);
                    if (player.addObjet(obj1, true))
                        Mundo.addGameObject(obj1, true);
            ...
            case 521://Echange clef skeunk
                ...
                if (player.hasItemTemplate(7887, 1) && player.hasItemTemplate(7888, 1) && player.hasItemTemplate(7889, 1) && player.hasItemTemplate(7890, 1)) {
                    ...
                    obj1 = Mundo.mundo.getObjetoModelo(8073).createNewItem(1, false);
                    if (player.addObjet(obj1, true))
                        Mundo.addGameObject(obj1, true);
            ...
            case 522://P�ki p�ki
                ...
                obj1 = Mundo.mundo.getObjetoModelo(6978).createNewItem(1, false);
                if (player.addObjet(obj1, true))
                    Mundo.addGameObject(obj1, true);
            ...
            case 523://Cawotte vs spell
                ...
                if (player.hasItemTemplate(361, 100)) {
                    ...
                    NpcPregunta quest = Mundo.mundo.getNPCQuestion(473);
                    if (quest == null) {
                        GestorSalida.GAME_SEND_END_DIALOG_PACKET(client);
                        player.setExchangeAction(null);
                        return true;
            ...
            case 524://R�ponse Maitre corbac
                if(client == null) return true;
                int qID = Monstruos.MobGroup.MAITRE_CORBAC.check();
                NpcPregunta quest = Mundo.mundo.getNPCQuestion(qID);
                if (quest == null) {
                    GestorSalida.GAME_SEND_END_DIALOG_PACKET(client);
                    player.setExchangeAction(null);
            ...
            case 525://EndFight Action Maitre Corbac
                Monstruos.MobGroup group = player.hasMobGroup();

                split = args.split(";");

                for (Monstruos.MobGrade mb : group.getMobs().values()) {
            ...
            case 526://Fin donjon maitre corbac
                ...
                obj1 = Mundo.mundo.getObjetoModelo(7703).createNewItem(1, false);
                if (player.addObjet(obj1, true))
                    Mundo.addGameObject(obj1, true);
            ...
            case 964://Signer le registre
                ...
                if (player.hasItemTemplate(9487, 1)) {
                    String date = player.getItemTemplate(9487, 1).getTxtStat().get(Constantes.STATS_DATE);
                    long timeStamp = Long.parseLong(date);
                    if (Instant.now().toEpochMilli() - timeStamp <= 1209600000) // 14 jours
                    {
                        return true;
                    } else {
                        player.removeByTemplateID(9487, 1);
                        GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 1 + "~" + 9487);
                    }
                }
                ...
                if (player.hasItemTemplate(9811, 1)) // Formulaire de neutralit�
                {
                    ...
                    GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 1 + "~" + 9811);
                ...
                } else if (player.hasItemTemplate(9812, 1)) // Formulaire de d�sertion
                {
                    if (player.hasItemTemplate(9488, 1)) {
                        ...
                        GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 1 + "~" + 9488);
                        player.modifAlignement(1);
                    } else if (player.hasItemTemplate(9489, 1)) {
                        player.removeByTemplateID(9489, 1);
                        GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 1 + "~" + 9489);
                        player.modifAlignement(2);
                    }
                    player.removeByTemplateID(9812, 1);
                    GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 1 + "~" + 9812);
                }

                ObjetoModelo t2 = Mundo.mundo.getObjetoModelo(9487);
                ObjetoJuego obj2 = t2.createNewItem(1, false);
                obj2.refreshStatsObjet("325#0#0#"
                        + Instant.now().toEpochMilli());
                if (player.addObjet(obj2, false)) {
                    GestorSalida.GAME_SEND_Im_PACKET(player, "021;" + 1
                            + "~" + obj2.getModelo().getId());
                    Mundo.addGameObject(obj2, true);
                }

                quest = Mundo.mundo.getNPCQuestion(Integer.parseInt(this.args));
                if (quest == null) {
                    GestorSalida.GAME_SEND_END_DIALOG_PACKET(client);
                    player.setExchangeAction(null);
                    return true;
            ...
            case 965://Signer le document officiel
                ...
                } else if (player.hasItemTemplate(9812, 1)) // Formulaire de d�sertion
                {
                    ...
                    ObjetoModelo t = Mundo.mundo.getObjetoModelo(idTemp);
                    ObjetoJuego obj = t.createNewItem(1, false);
                    obj.refreshStatsObjet("325#0#0#"
                            + Instant.now().toEpochMilli());
            ...
            case 963://Formulaire de d�sertion
                ...
                t2 = Mundo.mundo.getObjetoModelo(9812);
            ...
            case 966://Formulaire de neutralit�
                ...
                if (player.getKamas() < kamas) {
                    GestorSalida.GAME_SEND_MESSAGE_SERVER(player, "10|" + kamas);
                    return true;
                } else {
                    player.setKamas(player.getKamas() - kamas);
                    GestorSalida.GAME_SEND_STATS_PACKET(player);
                    GestorSalida.GAME_SEND_Im_PACKET(player, "046;" + kamas);
                    ...
                    ObjetoModelo t = Mundo.mundo.getObjetoModelo(9811);
                    ObjetoJuego obj = t.createNewItem(1, false);
                    obj.refreshStatsObjet("325#0#0#"
                            + Instant.now().toEpochMilli());
                    if (player.addObjet(obj, false)) {
            ...
            case 967://Apprendre bricoleur
                ...
                Oficio job = Mundo.mundo.getMetier(65);
                ...
                if (player.getMetierByID(2) != null && player.getMetierByID(2).get_lvl() < 30 || player.getMetierByID(11) != null && player.getMetierByID(11).get_lvl() < 30 || player.getMetierByID(13) != null && player.getMetierByID(13).get_lvl() < 30 || player.getMetierByID(14) != null && player.getMetierByID(14).get_lvl() < 30 || player.getMetierByID(15) != null && player.getMetierByID(15).get_lvl() < 30 || player.getMetierByID(16) != null && player.getMetierByID(16).get_lvl() < 30 || player.getMetierByID(17) != null && player.getMetierByID(17).get_lvl() < 30 || player.getMetierByID(18) != null && player.getMetierByID(18).get_lvl() < 30 || player.getMetierByID(19) != null && player.getMetierByID(19).get_lvl() < 30 || player.getMetierByID(20) != null && player.getMetierByID(20).get_lvl() < 30 || player.getMetierByID(24) != null && player.getMetierByID(24).get_lvl() < 30 || player.getMetierByID(25) != null && player.getMetierByID(25).get_lvl() < 30 || player.getMetierByID(26) != null && player.getMetierByID(26).get_lvl() < 30 || player.getMetierByID(27) != null && player.getMetierByID(27).get_lvl() < 30 || player.getMetierByID(28) != null && player.getMetierByID(28).get_lvl() < 30 || player.getMetierByID(31) != null && player.getMetierByID(31).get_lvl() < 30 || player.getMetierByID(36) != null && player.getMetierByID(36).get_lvl() < 30 || player.getMetierByID(41) != null && player.getMetierByID(41).get_lvl() < 30 || player.getMetierByID(56) != null && player.getMetierByID(56).get_lvl() < 30 || player.getMetierByID(58) != null && player.getMetierByID(58).get_lvl() < 30 || player.getMetierByID(60) != null && player.getMetierByID(60).get_lvl() < 30) {
                    GestorSalida.send(client, "DQ336|4840");
                    return false;
                }
                ...
                if (player.totalJobBasic() > 2) {
                    ...
                    if (player.hasItemTemplate(459, 20) && player.hasItemTemplate(7657, 15)) {
                        GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 20 + "~" + 459);
                        player.removeByTemplateID(459, 20);
                        GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 15 + "~" + 7657);
                        player.removeByTemplateID(7657, 15);
                        player.learnJob(job);
                        GestorSalida.send(client, "DQ3153|4840");
                        return false;
                    } else {
                        GestorSalida.send(client, "DQ3151|4840");
                        return false;
                    }
                }
                return true;
                ...
                if (player.getCurMap().getId() == (short) 9788) {
                    ...
                    if (key0 || player.hasItemTemplate(10207, 1)) {
                        ...
                        if(player.hasItemTemplate(10207, 1)) {
                            String stats = player.getItemTemplate(10207).getTxtStat().get(Constantes.STATS_NAME_DJ);
                            ...
                            if(key1 && key2){
                                ...
                                String replace2 = Integer.toHexString(8343);
                                StringBuilder newStats = new StringBuilder();
                                for (String i : stats.split(","))
                                    newStats.append((newStats.length() == 0) ? i : "," + i);
                                player.getItemTemplate(10207).getTxtStat().remove(Constantes.STATS_NAME_DJ);
                                player.getItemTemplate(10207).getTxtStat().put(Constantes.STATS_NAME_DJ, newStats.toString());
                        ...
                        if(key0 || (key1 && key2)) {
                            GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 1 + "~" + 8342);
                            GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 1 + "~" + 8343);
            ...
            case 972://Sortir du donjon skeunk avec le trousseau
                ...
                GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 1 + "~" + 7935);
                player.removeByTemplateID(7935, 1);
                GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 1 + "~" + 7936);
                player.removeByTemplateID(7936, 1);
                GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 1 + "~" + 7937);
                player.removeByTemplateID(7937, 1);
                GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 1 + "~" + 7938);
                player.removeByTemplateID(7938, 1);
                GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 1 + "~" + 8073);

                ObjetoJuego object = Mundo.mundo.getObjetoModelo(8072).createNewItem(1, false);

                if (player.addObjet(object, false)) {
                    GestorSalida.GAME_SEND_Im_PACKET(player, "021;" + 1 + "~" + object.getModelo().getId());
            ...
            case 973://Sortir du donjon skeunk avec la clef
                ...
                ObjetoModelo dofus = Mundo.mundo.getObjetoModelo(8072);
                ObjetoJuego obj = dofus.createNewItem(1, false);
                if (player.addObjet(obj, false)) {
                    GestorSalida.GAME_SEND_Im_PACKET(player, "021;" + 1 + "~" + obj.getModelo().getId());
                    Mundo.addGameObject(obj, true);
                }
            ...
            case 977://T�l�portation en salle des dalles Toror
                try {
                    switch (player.getCurMap().getId()) {
                        case 9553, 9554, 9555, 9556, 9557, 9558, 9559, 9560, 9561, 9562, 9563, 9564, 9565, 9566, 9567, 9568, 9569, 9570, 9571, 9572, 9573, 9574, 9575, 9576, 9577 -> player.teleport((short) 9876, 287);
                    }
                } catch (Exception e) {
                    return true;
                }
            ...
            case 978://T�l�portation en salle des dalles DC
                try {
                    switch (player.getCurMap().getId()) {
                        case 9372, 9384, 9380, 9381, 9382, 9383, 9393, 9374, 9394, 9390, 9391, 9392, 9373, 9389, 9385, 9386, 9387, 9388, 9371, 9375, 9376, 9377, 9378, 9379 -> player.teleport((short) 9396, 387);
                    }
                } catch (Exception e) {
                    return true;
                }
            ...
            case 979://T�l�portation labyrinth DC
                try {
                    short newMapID = Short.parseShort(args.split(",", 2)[0]);
                    final Mapa newMap = Mundo.mundo.getMap(newMapID);
                    int newCellID = Integer.parseInt(args.split(",", 2)[1]);
                    final GameCase curCase = player.getCurCell();
                    final Mapa curMap = player.getCurMap();
                    int idCurCase = curCase.getId();
                    if (idCurCase < 52 || idCurCase > 412) // On monte ou on descend
                    {
                        player.teleportLaby(newMapID, newCellID);
                        Temporizador.addSiguiente(() -> {
                            DragoCerdo.close(curMap, curCase);
                            DragoCerdo.close(newMap, DragoCerdo.getDownCell(newMap));
                            DragoCerdo.close(newMap, DragoCerdo.getUpCell(newMap));
                            DragoCerdo.open(newMap, DragoCerdo.getRightCell(newMap));
                            DragoCerdo.open(newMap, DragoCerdo.getLeftCell(newMap));
                        }, 1000, Temporizador.DataType.MAPA);
                    } else if (idCurCase == 262 || idCurCase == 320 || idCurCase == 144 || idCurCase == 216 || idCurCase == 231 || idCurCase == 274) // A gauche ou a droite
                    {
                        ...
                        Temporizador.addSiguiente(() -> {
                            DragoCerdo.close(curMap, curCase);
                            DragoCerdo.close(newMap, DragoCerdo.getLeftCell(newMap));
                            DragoCerdo.close(newMap, DragoCerdo.getRightCell(newMap));
                            DragoCerdo.open(newMap, DragoCerdo.getUpCell(newMap));
                            DragoCerdo.open(newMap, DragoCerdo.getDownCell(newMap));
                        }, 1000, Temporizador.DataType.MAPA);
                    } else {
                        GestorSalida.GAME_SEND_MESSAGE(player, "Cette porte n'est pas fonctionnelle. Veuillez reporter la map et la porte sur le forum.");
                        return true;
                    }
                } catch (Exception e) {
                    return true;
            ...
            case 980: // t�l�portation avec mapsecu, et deux itemssecu supprim�s : donjon dc
                try {
                    ...
                    GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 1 + "~" + item);
                    player.removeByTemplateID(item, 1);
                    GestorSalida.GAME_SEND_Im_PACKET(player, "022;" + 1 + "~" + item2);
            ...
            case 983:
                try {
                    Mision q = Mision.getQuestById(193);
                    if (q == null)
                        return true;
                    ...
                    Monstruos petitChef = Mundo.mundo.getMonstre(984);
                    if (petitChef == null)
                        return true;
                    Monstruos.MobGrade mg = petitChef.getGradeByLevel(10);
                    if (mg == null)
                        return true;
                    Monstruos.MobGroup _mg = new Monstruos.MobGroup(player.getCurMap().nextObjectId, player.getCurCell().getId(), petitChef.getId() + "," + mg.getLevel() + "," + mg.getLevel() + ";");
            ...
            case 984:
                try {
                    ...
                    MisionJugador qp = player.getQuestPersoByQuestId(idQuest);
                    if (qp == null)
                        return true;
                    if (qp.isFinish())
                        return true;
                    ...
                    GestorSalida.GAME_SEND_Im_PACKET(player, "08;" + xp);
                    qp.setFinish(true);
                    GestorSalida.GAME_SEND_Im_PACKET(player, "055;" + idQuest);
                    GestorSalida.GAME_SEND_Im_PACKET(player, "056;" + idQuest);
                } catch (Exception e) {
                    e.printStackTrace();
                }
                break;
            ...
            case 985:
                ...
                try {
                    ...
                    if (player.getCurMap().getId() != (short) mapCurId)
                        return true;
                    Oficio metierArgs = Mundo.mundo.getMetier(metierId);
                    if (metierArgs == null)
                        return true;
                    ...
                    ObjetoModelo t = Mundo.mundo.getObjetoModelo(item2);
                    if (t == null)
                        return true;

                    if (player.hasItemTemplate(item, 1)) {

                        for (Entry<Integer, OficioCaracteristicas> entry : player.getMetiers().entrySet()) {
                        ...
                        player.learnJob(Mundo.mundo.getMetier(metierId));
                        Database.dinamicos.getPlayerData().update(player);
                        GestorSalida.GAME_SEND_Ow_PACKET(player);
                    }
                } catch (Exception e) {
            ...
            case 987:
                ...
                try {
                    ...
                    if (player.hasItemTemplate(item, 1)
                        ...
                        ObjetoModelo t = Mundo.mundo.getObjetoModelo(item3);
                        if (t != null) {
                            obj = t.createNewItem(1, false);
                            if (player.addObjet(obj, false)) {
                                GestorSalida.GAME_SEND_Im_PACKET(player, "021;"
            ...
            case 988: // devenir p�cheur
                ...
                try {
                    if (player.hasItemTemplate(2107, 1)) {
                        long timeStamp = Long.parseLong(player.getItemTemplate(2107, 1).getTxtStat().get(Constantes.STATS_DATE));
                        boolean success = (Instant.now().toEpochMilli()
                                - timeStamp <= 2 * 60 * 1000);
                        NpcPregunta qQuest = Mundo.mundo.getNPCQuestion(success ? 1171 : 1172);
                        ...
                        if (success) {
                            Oficio metierArgs = Mundo.mundo.getMetier(36);
                            if (metierArgs == null)
                            ...
                            player.learnJob(Mundo.mundo.getMetier(36));
                            Database.dinamicos.getPlayerData().update(player);
                            GestorSalida.GAME_SEND_Ow_PACKET(player);
                        }

                        GestorSalida.GAME_SEND_QUESTION_PACKET(client, qQuest.parse(player));
                        return false;
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                ...
                try {
                    if (player.getCurMap().getId() == (short) 7388) {
                        if (player.hasItemTemplate(2039, 1)
                            ...
                            long timeStamp = Long.parseLong(player.getItemTemplate(2039, 1).getTxtStat().get(Constantes.STATS_DATE));
                            boolean success = (Instant.now().toEpochMilli()
                                    - timeStamp <= 2 * 60 * 1000);
                            NpcPregunta qQuest = Mundo.mundo.getNPCQuestion(success ? 2364 : 1175);
                            ...
                            if (success) {
                                Oficio metierArgs = Mundo.mundo.getMetier(41);
                                if (metierArgs == null)
                                    return true; // Si le m�tier n'existe pas
                                ...
                                player.learnJob(Mundo.mundo.getMetier(41));
                                Database.dinamicos.getPlayerData().update(player);
                                GestorSalida.GAME_SEND_Ow_PACKET(player);
                            }
                    ...
                    if (player.getCurMap().getId() == (short) mapCurId) {
                        if (player.hasItemTemplate(item, 1)) {
                            String groupe = monstre + "," + grade + "," + grade
                                    + ";";
                            Monstruos.MobGroup Mgroupe = new Monstruos.MobGroup(player.getCurMap().nextObjectId, player.getCurCell().getId(), groupe);
            ...
            case 992: // Supprime deux items & apprends un m�tier
                ...
                try {
                    ...
                    if (player.getCurMap().getId() == (short) mapCurId) {
                        ...
                        Oficio metierArgs = Mundo.mundo.getMetier(mId);
                        if (metierArgs == null)
                            return true;
            ...
            case 996: // t�l�portation mine chariot + Animation
                Mapa curMap = player.getCurMap();
                ArrayList<Integer> mapSecure = new ArrayList<>();
                for (String i : args.split(","))
                ...
                if (!player.isInPrison()) {
                    ...
                    if (player.isOnline())
                        GestorSalida.GAME_SEND_STATS_PACKET(player);
                    GestorSalida.GAME_SEND_Im_PACKET(player, "046;" + 50);
            ...
            case 997: // Apprendre un m�tier de forgemagie
                try {
                    ...
                    Mapa curMapPerso = player.getCurMap();
                    if (curMapPerso.getId() != (short) mapIdargs)
                        return true; // Map secure

                    if (metierArgs.isMaging()) // Si c'est du FM
                    {
                        OficioCaracteristicas metierBase = player.getMetierByID(Mundo.mundo.getMetierByMaging(metierID));
                        if (metierBase == null)
            ...
            case 998://Donjon abraknyde salle des cases
                if (player.getCurMap().getId() == 10154
                    ...
                    GestorSalida.GAME_SEND_MESSAGE(player, "No estás en la celda adecuada para ser teleportado.");
                }
                break;
            ...
            case 1003: // Item x bendicion (Dojos pandala) - IDOBJETO, CANTIDAD, BENDICION

                    int objeto = Integer.parseInt(args.split(",")[0]);
                    int cantidad = Integer.parseInt(args.split(",")[1]);
                    int bendicion = Integer.parseInt(args.split(",")[2]);

                    if(player.hasItemTemplate(objeto, cantidad)) {
                    player.setBenediction(bendicion);
            }
            GestorSalida.GAME_SEND_Im_PACKET(player, "1266;" + cantidad + "~" + Mundo.mundo.getObjetoModelo(objeto).getName());
                break;

            case 1004: //Accion para NPC cliqueador
                try {
                    int stat = Integer.parseInt(args);
                    int value = 0;
                    int cout =0;
                    while(cout <= player.get_capital()){
                        switch (stat) {
                            //Fuerza
                            case 10 -> value = player.getCaracteristicas().getEffect(Constantes.STATS_ADD_FORC);
                            //Suerte
                            case 13 -> value = player.getCaracteristicas().getEffect(Constantes.STATS_ADD_CHAN);
                            //Agilidad
                            case 14 -> value = player.getCaracteristicas().getEffect(Constantes.STATS_ADD_AGIL);
                            //Inteligencia
                            case 15 -> value = player.getCaracteristicas().getEffect(Constantes.STATS_ADD_INTE);
                        }
                        cout = Constantes.getReqPtsToBoostStatsByClass(player.getClasse(), stat, value);
                        switch(stat) {
                            case 11://Vitalidad
                                if(player.getClasse() != Constantes.CLASE_SACROGRITO)
                                    player.getCaracteristicas().addOneStat(Constantes.STATS_ADD_VITA, 1);
                                else
                                    player.getCaracteristicas().addOneStat(Constantes.STATS_ADD_VITA, 2);
                                break;
                            case 12://Sabiduria
                                player.getCaracteristicas().addOneStat(Constantes.STATS_ADD_SAGE, 1);
                                break;
                            case 10://Fuerza
                                player.getCaracteristicas().addOneStat(Constantes.STATS_ADD_FORC, 1);
                                break;
                            case 13://Suerte
                                player.getCaracteristicas().addOneStat(Constantes.STATS_ADD_CHAN, 1);
                                break;
                            case 14://Agilidad
                                player.getCaracteristicas().addOneStat(Constantes.STATS_ADD_AGIL, 1);
                                break;
                            case 15://Inteligencia
                                player.getCaracteristicas().addOneStat(Constantes.STATS_ADD_INTE, 1);
                                break;
                            default:
                                return true;
                        }
                        player.addCapital(-cout);
                    }
                    GestorSalida.GAME_SEND_MESSAGE(player, "Tus puntos de capital se han distribuido");
                    GestorSalida.GAME_SEND_STATS_PACKET(player);
                    Database.dinamicos.getPlayerData().update(player);
                }catch(Exception ignored){}
                break;

            case 1005: //Transportar a los miembros del grupo - Entrada Mazmorra
                Grupo grupo = player.getParty();
                //Verificamos que el jugador este en un grupo
                if (grupo == null) {
                    GestorSalida.GAME_SEND_Im_PACKET(player, "1251;");
                    return true;
                }
                //Vemos la ID del mapa y celda del jugador a crear la accion
                short idmapa = player.getCurMap().getId();
                int idcelda = player.getCurCell().getId();
                //Listamos los miembros del grupo
                ArrayList<Jugador> miembros = player.getParty().getPlayers();
                for (Jugador p : miembros) {
                    p.teleport(idmapa, idcelda);
                }
                break;

            case 1006://Fin de dung Kimbo + Recompensa
                if (player.getCurMap().getId() != 11518)
                    return true;

                obj1 = Mundo.mundo.getObjetoModelo(8675).createNewItem(1, false);
                if (player.addObjet(obj1, true))
                    Mundo.addGameObject(obj1, true);
                player.send("Im021;1~8675");
                player.teleport((short) 11136, 295);
                break;
