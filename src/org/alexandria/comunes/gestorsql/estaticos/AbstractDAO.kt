...
abstract class AbstractDAO<T>(protected var dataSource: HikariDataSource) : DAO<T> {
    ...
    protected var logger =
        LoggerFactory.getLogger(AbstractDAO::class.java.toString() + " - [D]") as Logger
    ...
    protected fun getPreparedStatement(query: String?): PreparedStatement? {
        return try {
            ...
            dataSource.close()
            if (Database.estaticos.inicializarconexion()) stop("PreparedStatement failed")
            null
        }
    }
    ...
    protected fun sendError(msg: String, e: Exception) {
        e.printStackTrace()
        logger.error("Error dynamics database " + msg + " : " + e.message)
    }
