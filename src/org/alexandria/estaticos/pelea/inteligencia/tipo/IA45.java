 ...
 */
public class IA45 extends NecesitaHechizo {

    private byte attack = 0;

    public IA45(Pelea fight, Peleador fighter, byte count) {
        ...
        if (!this.stop && this.fighter.canPlay() && this.count > 0) {
            ...
            if(this.fighter.getCurPa(this.fight) > 0 && C != null && !action) {
                ...
                if(value != 0) {
                    ...
                    value = Funcion.getInstance().moveautourIfPossible(this.fight, this.fighter, E);
                    if(value != 0) {
                        time = value;
                        action = true;
                        C = Funcion.getInstance().getNearestEnnemynbrcasemax(this.fight, this.fighter, 0, maxPo + 1);
