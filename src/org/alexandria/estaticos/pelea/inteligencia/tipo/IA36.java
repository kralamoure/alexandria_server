 ...
 */
public class IA36 extends NecesitaHechizo {

    public IA36(Pelea fight, Peleador fighter, byte count) {
    ...
    public void apply() {
        if (!this.stop && this.fighter.canPlay() && this.count > 0) {
            ...
            if(this.fighter.getCurPa(this.fight) > 0 && L == null && C == null) {
                int nbrcase = Funcion.getInstance().attackBondIfPossible(this.fight, this.fighter, ennemy);
                if(nbrcase != 0) {
                    time = nbrcase;
                    action = true;
                    L = Funcion.getInstance().getNearestEnnemynbrcasemax(this.fight, this.fighter, 1, maxPo + 1);// pomax +1;
