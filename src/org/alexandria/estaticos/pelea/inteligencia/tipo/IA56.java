 ...
 */
public class IA56 extends NecesitaHechizo {

    public IA56(Pelea fight, Peleador fighter, byte count) {
    ...
    public void apply() {
        if (!this.stop && this.fighter.canPlay() && this.count > 0) {
            ...
            Peleador L = Funcion.getInstance().getNearestEnnemynbrcasemax(this.fight, this.fighter, 1, maxPo + 1 + this.fighter.getBuffValue(117));// pomax +1;
            ...
            if(this.fighter.getCurPm(this.fight) > 0 && L == null && C == null) {
                int value = Funcion.getInstance().moveenfaceIfPossible(this.fight, this.fighter, ennemy, maxPo + 1 + this.fighter.getBuffValue(117));
                if(value != 0) {
                    time = value;
                    action = true;
                    L = Funcion.getInstance().getNearestEnnemynbrcasemax(this.fight, this.fighter, 1, maxPo + 1 + this.fighter.getBuffValue(117));// pomax +1;
            ...
            if(this.fighter.getCurPa(this.fight) > 0 && L != null && C == null && !action) {
                int value = Funcion.getInstance().attackIfPossibleCM1(this.fight, this.fighter, this.cacs);
                if(value != 0) {
                    time = value;
                    action = true;
                }
