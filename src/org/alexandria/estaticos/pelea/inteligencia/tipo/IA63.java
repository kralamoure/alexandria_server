 ...
 */
public class IA63 extends NecesitaHechizo {

    public IA63(Pelea fight, Peleador fighter, byte count) {
    ...
    public void apply() {
        if (!this.stop && this.fighter.canPlay() && this.count > 0) {
            ...
            if(this.fighter.getCurPa(this.fight) > 0 && !action) {
                if (Funcion.getInstance().invocIfPossible(fight, this.fighter, this.invocations)) {
                    time = 1000;
                    action = true;
                }
            }
            if(this.fighter.getCurPa(this.fight) > 0 && !action) {
                if (Funcion.getInstance().HealIfPossible(fight, this.fighter, true, 50) != 0) {
                    time = 1000;
                    action = true;
                }
            }
