 ...
 */
public class IA55 extends NecesitaHechizo {

    private byte attack = 0, glyph = 0;

    public IA55(Pelea fight, Peleador fighter, byte count) {
    ...
    public void apply() {
        if (!this.stop && this.fighter.canPlay() && this.count > 0) {
            ...
            if(this.fighter.getCurPa(this.fight) > 0 && L != null && C == null && this.attack == 0) {
                int value = Funcion.getInstance().attackIfPossibleglyph(this.fight, this.fighter, L, this.glyphs);
                if(value != 0) {
                    this.glyph++;
                    time = value;
                    action = true;
