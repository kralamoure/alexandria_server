 ...
 */
public class IA25 extends NecesitaHechizo {

    public IA25(Pelea fight, Peleador fighter, byte count) {
    ...
    public void apply() {
        if (!this.stop && this.fighter.canPlay() && this.count > 0) {
            ...
            Peleador secondEnnemy = Funcion.getInstance().getNearestEnnemynbrcasemax(fight, this.fighter, 0, 5);//2 = po min 1 + 1;
            Peleador target = Funcion.getInstance().getNearestEnnemynbrcasemax(fight, this.fighter, 0, 2);//2 = po min 1 + 1;

            if(target != null) if(target.isHide()) target = null;

            if(this.fighter.getCurPm(this.fight) > 0 && secondEnnemy == null && target == null) {
                int num = Funcion.getInstance().moveautourIfPossible(fight, this.fighter, ennemy);
                if(num != 0) {
                    time = num;
                    action = true;
                    Funcion.getInstance().getNearestEnnemynbrcasemax(fight, this.fighter, 0, 5);//2 = po min 1 + 1;
                    target = Funcion.getInstance().getNearestEnnemynbrcasemax(fight, this.fighter, 0, 2);//2 = po min 1 + 1;
                }
            } else if(this.fighter.getCurPm(this.fight) > 0 && secondEnnemy != null && target == null) {
                int num = Funcion.getInstance().moveautourIfPossible(fight, this.fighter, secondEnnemy);
                if(num != 0) {
                    time = num;
                    action = true;
                    Funcion.getInstance().getNearestEnnemynbrcasemax(fight, this.fighter, 0, 5);//2 = po min 1 + 1;
