 ...
 */
public class IA41 extends NecesitaHechizo {

    private byte attack = 0;

    public IA41(Pelea fight, Peleador fighter, byte count) {
    ...
    public void apply() {
        if (!this.stop && this.fighter.canPlay() && this.count > 0) {
            ...
            Peleador D = Funcion.getInstance().getNearestEnnemynbrcasemax(this.fight, this.fighter, 0, 4);//po max+ 1;
            ...
            if(this.fighter.getCurPa(this.fight) > 0 && C != null && !action && D == null) {
                ...
                int value = Funcion.getInstance().attackIfPossibleWobot(this.fight, this.fighter);
                if(value != 0) {
                    time = value;
                    action = true;
                    this.attack++;
