 ...
 */
public class IA38 extends NecesitaHechizo {

    private boolean boost = false;

    public IA38(Pelea fight, Peleador fighter, byte count) {
    ...
    public void apply() {
        if (!this.stop && this.fighter.canPlay() && this.count > 0) {
            ...
            Peleador A = Funcion.getInstance().getNearestAminbrcasemax(this.fight, this.fighter, 0, maxPoBuff+1);//2 = po min 1 + 1;
            ...
            if(this.fighter.getCurPm(this.fight) > 0 && L == null && C == null && A == null && !this.boost) {
                int value = Funcion.getInstance().moveautourIfPossible(this.fight, this.fighter, Funcion.getInstance().getNearestAminbrcasemax(this.fight, this.fighter, 0, 100));
                if(value != 0) {
                    time = value;
                    action = true;
                    ...
                    A = Funcion.getInstance().getNearestAminbrcasemax(this.fight, this.fighter, 0, maxPoBuff+1);//2 = po min 1 + 1;
                    if(maxPo == 1) L = null;
                }
