 ...
 */
public class IA58 extends NecesitaHechizo {

    public IA58(Pelea fight, Peleador fighter, byte count) {
    ...
    public void apply() {
        if (!this.stop && this.fighter.canPlay() && this.count > 0) {
            ...
            if(this.fighter.getCurPm(this.fight) > 0 && L == null && C == null) {
                int nbrcase = Funcion.getInstance().moveautourIfPossible(this.fight, this.fighter, ennemy);
                if(nbrcase != 0) {
                    time = nbrcase;
                    action = true;
            ...
            if(this.fighter.getCurPa(this.fight) > 0 && C != null && !action) {
                int value = Funcion.getInstance().attackIfPossibleRN(this.fight, this.fighter, C);
                if(value != 0) {
                    time = value;
                    action = true;
                }
