...
public class TeamMatch {
    ...
    private final List<Jugador> firstTeam, secondTeam = new ArrayList<>();

    TeamMatch(Jugador player) {
    ...
    public void start() {
        ...
        Mapa map = Mundo.mundo.getMap(FightManager.maps[new Random().nextInt(FightManager.maps.length)]);
    ...
    public void unsubscribe(Jugador player) {
    ...
    public void unsubscribe(Grupo party) {
        party.getPlayers().forEach(player -> {
    ...
    boolean isAvailable(Jugador player, boolean group) {
        if(group) {
            ...
            if(((firstTeam.size() + secondTeam.size()) == PER_TEAM * 2))
                return false;
            for(Jugador other : getAllPlayers()) {
    ...
    public List<Jugador> getTeam(boolean first) {
    ...
    private short getTotalLevel(List<Jugador> players) {
        short average = 0;
